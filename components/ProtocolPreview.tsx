...
const ProtocolPreview: React.FC<Props> = ({ data }) => {
  // Classes para controle de página A4 rigoroso
  const fixedPageClass = "bg-white w-[210mm] h-[297mm] min-h-[297mm] mx-auto flex flex-col page-break text-black relative shadow-2xl print:shadow-none print:m-0 print:rounded-none mb-10 overflow-hidden";
  ...
  return (
    <div className="flex flex-col items-center w-full bg-transparent print:gap-0 print-container pb-20 print:pb-0">
      ...
      <button 
        onClick={handlePrint}
        className="no-print fixed bottom-8 right-8 z-[100] bg-[#d4af37] text-black px-8 py-5 rounded-full shadow-[0_0_50px_rgba(212,175,55,0.6)] hover:scale-110 transition-all font-black uppercase text-xs flex items-center gap-3 border-2 border-black/10"
      >
        <Printer size={20} /> Imprimir Protocolo A4
      </button>
      ...
      <div className={`${fixedPageClass} !bg-[#0a0a0a] !text-white border-b-[24px] border-[#d4af37] justify-between p-[2cm]`}>
        ...
        <div className="flex flex-col items-center pt-[3cm] w-full relative z-10">
          <div className="p-8 bg-white/5 rounded-[4rem] border border-white/10 backdrop-blur-xl mb-24 shadow-[0_0_80px_rgba(212,175,55,0.3)]">
            <img src={LOGO_RHINO_BLACK} alt="Logo Team VBR" className="w-80 h-auto" />
          </div>
          <div className="text-center space-y-10 w-full">
            <h1 className="text-7xl md:text-9xl font-montserrat font-black text-white tracking-tighter uppercase leading-[0.8]">
              PROTOCOLO <br/>
          ...
          <div className="mt-[4cm] text-center w-full max-w-3xl">
            ...
            <h2 className="text-6xl font-montserrat font-black text-white uppercase tracking-tight mb-6">{data.clientName || 'NOME DO ALUNO'}</h2>
            <div className="inline-block px-12 py-4 bg-[#d4af37] text-black rounded-full font-black uppercase tracking-[0.3em] text-[14px]">
      ...
      <div className={`${fixedPageClass} p-[1.5cm]`}>
        <div className="flex justify-between items-end border-b-8 border-[#d4af37] pb-10 mb-14">
          ...
          <div className="text-right"><span className="text-2xl font-black text-gray-400">{new Date().toLocaleDateString('pt-BR')}</span></div>
        </div>
        <div className="grid grid-cols-4 gap-6 mb-12">
          {[{ label: 'Peso', value: data.physicalData.weight, unit: 'kg' }, { label: 'Altura', value: data.physicalData.height, unit: 'm' }, { label: 'Idade', value: data.physicalData.age, unit: 'anos' }, { label: 'BF%', value: data.physicalData.bodyFat, unit: '%' }].map((item, idx) => (
            <div key={idx} className="bg-gray-50 p-8 rounded-[2.5rem] text-center border border-gray-100 avoid-break shadow-sm">
              ...
              <div className="text-5xl font-black text-black leading-none">{item.value || '--'}<span className="text-sm ml-1 text-[#d4af37]">{item.unit}</span></div>
            </div>
          ))}
        </div>
        <div className="grid grid-cols-2 gap-10 mb-12 flex-1">
          ...
          <div className="bg-[#0a0a0a] text-white p-12 rounded-[4rem] flex flex-col justify-center border-r-[12px] border-[#d4af37] avoid-break">
            <h3 className="text-[11px] font-black uppercase tracking-[0.5em] text-[#d4af37] mb-8">Estratégia do Coach</h3>
            <p className="text-[20px] leading-relaxed italic text-white/95 font-medium">"{data.physicalData.observations || "Foco absoluto em performance."}"</p>
          </div>
        </div>
        <div className="bg-[#0a0a0a] p-10 rounded-[3.5rem] text-white flex flex-col items-center border-b-8 border-[#d4af37] mt-auto">
          <span className="text-[11px] font-black uppercase tracking-[0.6em] text-[#d4af37] mb-3">OBJETIVO CENTRAL</span>
          <div className="text-5xl font-black font-montserrat uppercase text-center">{data.protocolTitle || 'ALTA PERFORMANCE'}</div>
        </div>
      </div>
      ...
      <div className={`${fixedPageClass} p-[1.5cm]`}>
        ...
        <div className="bg-[#0a0a0a] text-white p-14 rounded-[5rem] flex flex-col items-center mb-12 shadow-2xl border-b-[12px] border-[#d4af37] avoid-break">
          <span className="text-[16px] font-black text-[#d4af37] mb-8 tracking-[1em] uppercase">Meta Calórica</span>
          <div className="text-[110px] font-black font-montserrat leading-none">{data.kcalGoal || '0000'} <span className="text-3xl text-[#d4af37]">KCAL</span></div>
        </div>
        <div className="grid grid-cols-3 gap-8 mb-12">
          {[{ label: 'Proteínas', val: data.macros.protein.value, unit: 'g', color: 'bg-red-600' }, { label: 'Carbos', val: data.macros.carbs.value, unit: 'g', color: 'bg-blue-600' }, { label: 'Gorduras', val: data.macros.fats.value, unit: 'g', color: 'bg-amber-600' }].map((macro, i) => (
            <div key={i} className="bg-white p-10 rounded-[3.5rem] border border-gray-100 shadow-lg text-center avoid-break">
              <div className={`w-5 h-5 rounded-full ${macro.color} mx-auto mb-5`}></div>
              <span className="text-gray-400 font-black uppercase text-[12px] mb-3 block">{macro.label}</span>
              <div className="text-5xl font-black mb-2">{macro.val || '0'}{macro.unit}</div>
            </div>
          ))}
        </div>
        <div className="bg-gray-50 p-12 rounded-[4rem] border-l-[14px] border-[#d4af37] mt-auto avoid-break">
          <h4 className="font-black uppercase text-[12px] text-[#d4af37] mb-5">Metodologia Nutricional</h4>
          <p className="text-[22px] leading-relaxed text-gray-800 italic font-bold">"{data.nutritionalStrategy || "Estratégia personalizada."}"</p>
        </div>
      </div>
      ...
      <div className={`${dynamicPageClass} p-[1.5cm]`}>
        <div className="flex justify-between items-end border-b-8 border-[#d4af37] pb-10 mb-14">
          <div>
             ...
             <h2 className="text-6xl font-black uppercase font-montserrat tracking-tighter text-black leading-none">Dieta Diária</h2>
          </div>
        </div>
        <div className="rounded-[4rem] border border-gray-100 overflow-hidden shadow-2xl mb-12">
          <table className="w-full">
            <thead className="bg-[#0a0a0a] text-white">
              <tr>
                <th className="p-10 text-left font-black uppercase text-[13px] tracking-[0.4em] text-[#d4af37] w-48 border-r border-white/10">Hora</th>
                <th className="p-10 text-left font-black uppercase text-[13px] tracking-[0.4em]">Refeição</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {data.meals.map((meal) => (
                <tr key={meal.id} className="avoid-break">
                  ...
                  <td className="p-12">
                    <div className="font-black text-3xl text-black mb-5 uppercase">{meal.name}</div>
      ...
      <div className={`${dynamicPageClass} p-[1.5cm]`}>
        <div className="flex justify-between items-end border-b-8 border-[#d4af37] pb-10 mb-14">
          <div>
            ...
            <h2 className="text-6xl font-black uppercase font-montserrat tracking-tighter text-black leading-none">Treinamento</h2>
          </div>
        </div>
          ...
          {data.trainingDays.map((day) => (
            <div key={day.id} className="rounded-[4rem] border border-gray-100 overflow-hidden shadow-2xl avoid-break">
               <div className="bg-[#0a0a0a] text-white p-8 flex justify-between items-center border-b-10 border-[#d4af37]">
                  <h3 className="text-4xl font-black text-[#d4af37] uppercase font-montserrat">{day.title}</h3>
                  <span className="bg-[#d4af37] text-black px-8 py-2 rounded-2xl text-[12px] font-black uppercase">{day.focus}</span>
               </div>
               <table className="w-full">
                 <tbody className="divide-y divide-gray-100">
      ...
      <div className={`${fixedPageClass} !bg-[#0a0a0a] !text-white justify-center p-[2cm] border-t-[24px] border-[#d4af37]`}>
        <div className="max-w-3xl text-center space-y-20 relative z-10 mx-auto">
           <div className="bg-white/5 border-2 border-[#d4af37]/40 p-20 rounded-[5rem] backdrop-blur-2xl shadow-[0_0_100px_rgba(212,175,55,0.2)]">
              <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-[#d4af37] text-black px-12 py-3 rounded-full font-black uppercase text-[14px]">CONCLUSÃO DO COACH</div>
              <p className="text-4xl leading-tight font-black italic text-white/95 uppercase font-montserrat">
                "{data.generalObservations || "CONSTÂNCIA É A CHAVE."}"
              </p>
           </div>
           <div className="pt-24 flex flex-col items-center">
             <img src={LOGO_RHINO_BLACK} alt="Logo" className="w-80 mx-auto mb-16" />
             <p className="text-5xl font-black tracking-[0.7em] uppercase font-montserrat">TEAM VBR</p>
             <div className="w-64 h-1.5 bg-[#d4af37] my-10"></div>
             <p className="text-[16px] font-black text-white/40 tracking-[1.8em] uppercase">SEM DESCULPAS</p>
           </div>
        </div>
      </div>
    </div>
