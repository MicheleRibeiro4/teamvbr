    ...
    <div className="flex flex-col items-center w-full bg-transparent print:gap-0 print-container pb-20 print:pb-0">
      ...
      <div className={`${fixedPageClass} !bg-[#0a0a0a] !text-white border-b-[20px] border-[#d4af37] justify-center items-center p-[2cm]`}>
        <div className="flex flex-col items-center w-full text-center">
          ...
          <div className="mt-10">
            <p className="text-white/80 font-bold text-lg">{data.consultantName}</p>
        ...
        <div className="space-y-10">
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-6">
              {[
                { label: 'PROTEÍNAS', val: data.macros.protein.value, sub: data.macros.protein.ratio || '≈ 2,2g/kg', color: 'text-amber-600' },
                { label: 'CARBOIDRATOS', val: data.macros.carbs.value, sub: data.macros.carbs.ratio || 'Energia', color: 'text-amber-600' },
                { label: 'GORDURAS', val: data.macros.fats.value, sub: data.macros.fats.ratio || 'Regulação', color: 'text-amber-600' }
              ].map((m, i) => (
                <div key={i} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-xl text-center">
            ...
            <tbody className="divide-y divide-gray-100">
              {data.meals.map((meal) => (
                <tr key={meal.id} className="avoid-break">
                  <td className="p-6 font-black text-[#d4af37] text-3xl align-top">{meal.time}</td>
                  <td className="p-6">
      ...
      <div className={`${fixedPageClass} p-[1.5cm]`}>
        ...
        <div className="space-y-6 mb-16">
          {data.supplements.map((s, idx) => {
            ...
            return (
              <div key={s.id} className={`${colors[idx % 3]} text-white p-8 rounded-[2.5rem] flex justify-between items-center shadow-xl`}>
                <div>
                   <h3 className="text-3xl font-black uppercase mb-1">{s.name}</h3>
                   <p className="text-xl font-bold opacity-90">{s.dosage}</p>
                </div>
                <div className="bg-white/20 backdrop-blur-md px-6 py-2 rounded-2xl font-black uppercase text-sm">
                   {s.timing}
                </div>
              </div>
            );
      ...
      {data.trainingDays.length > 2 && (
        <div className={`${dynamicPageClass} p-[1.5cm]`}>
          ...
          <div className="space-y-10">
            {data.trainingDays.slice(2, 4).map((day) => (
              <div key={day.id} className="rounded-[2rem] border border-gray-100 overflow-hidden shadow-xl avoid-break">
                <div className="bg-[#1a1a1a] text-white p-6 flex justify-between items-center border-b-4 border-[#d4af37]">
                  <h3 className="text-2xl font-black text-[#d4af37] uppercase font-montserrat">{day.title}</h3>
                  <span className="bg-[#d4af37] text-black px-6 py-1.5 rounded-xl text-sm font-black uppercase">{day.focus}</span>
      ...
      {/* PÁGINA 7: ATENÇÃO */}
      <div className={`${fixedPageClass} justify-center items-center p-[2cm]`}>
