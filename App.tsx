  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans selection:bg-[#d4af37] selection:text-black">
        ...
        <div className="flex items-center gap-2 md:gap-4">
          {data.id && activeView !== 'home' && activeView !== 'search' && (
            <button onClick={() => handleSave()} disabled={isSyncing} className="flex items-center gap-2 bg-[#d4af37] text-black px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest hover:scale-105 transition-all shadow-lg">
              {isSyncing ? <RefreshCw size={16} className="animate-spin" /> : <Database size={16} />} Salvar Alterações
            </button>
          )}
        </div>
      </header>
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-10">
        {activeView === 'home' && <MainDashboard protocols={savedProtocols} onNew={handleNew} onList={() => setActiveView('search')} onLoadStudent={(p) => { setData(p); setActiveView('student-dashboard'); }} />}
        {activeView === 'search' && <StudentSearch protocols={savedProtocols} onLoad={(p) => { setData(p); setActiveView('student-dashboard'); }} onDelete={async (id) => { await db.deleteProtocol(id); setSavedProtocols(prev => prev.filter(p => p.id !== id)); }} />}
        ...
        {activeView === 'evolution' && (
          <div className="space-y-10">
            <button onClick={() => setActiveView('student-dashboard')} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-[#d4af37] transition-colors"><ChevronLeft size={16} /> Voltar ao Painel</button>
            <EvolutionTracker currentProtocol={data} history={savedProtocols.filter(p => p.clientName === data.clientName)} onNotesChange={(n) => setData({...data, privateNotes: n})} onUpdateData={(newData) => { setData(newData); setTimeout(() => handleSave(true), 100); }} />
          </div>
        )}
      </main>
    </div>
